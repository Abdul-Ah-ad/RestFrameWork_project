...
class TeamViewSet(viewsets.ModelViewSet):
    ...
    endpoint for uploading team/player data via a JSON file.
    ...
    def upload_team_data(self, request):
        ...
        Returns:
            - 201 CREATED: If new data is imported or existing data is updated.
        ...
        if not uploaded_file.name.endswith('.json'):
            return Response(
                {'error': '❌ Only .json files are supported.'},status=status.HTTP_400_BAD_REQUEST)

        try:
            parsed_data = json.loads(uploaded_file.read().decode('utf-8'))
        except (UnicodeDecodeError, json.JSONDecodeError) as decode_err:
            logger.error(f'Failed to parse uploaded file: {decode_err}')
            return Response(
                {'error': f'❌ Failed to parse JSON: {str(decode_err)}'},status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            logger.exception('Unexpected error during file upload.')
            return Response(
                {'error': f'❌ Unexpected error: {str(e)}'},status=status.HTTP_400_BAD_REQUEST)
